...
async function runOnce() {
  ...
  try {
    ...
    if (addButtons.length > 0) {
      try { await addButtons[0].click(); await page.waitForTimeout(1500); } catch {}
    }
    ...
    try { await page.getByRole('button', { name: /Reveal/i }).click({ timeout: 3000 }); } catch {}
